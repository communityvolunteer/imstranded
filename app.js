...
function renderFilteredPosts(map, cluster, filteredPosts) {
  ...
  for (const p of filteredPosts) {
    ...
    const popupHtml = `<div style="font-family:Inter,sans-serif">
        ...
        ${shareIcon(shareRoomText(p), 'room='+p.id, 'Share this room offer')}
      </div>`;
  ...
  for (const p of filteredData) {
    ...
    const popupHtml = `
      <div style="font-family:Inter,sans-serif">
        ...
        ${shareIcon(shareStrandedText(p), '', 'Share to help this person')}
      </div>
    `;
    if (isMobile) {
...
function setAccent(name) {
  ...
  document.querySelectorAll('[style]').forEach(el => {
    ...
    if (el.hasAttribute('data-accent-lock') || el.closest('[data-accent-lock]')) return;
  ...
  for (const p of _petPosts) {
    ...
    const popHtml = `
      <div style="font-family:Inter,sans-serif">
        ...
        ${buildFlagButton('stranded_pets', p.id)}
        ${shareIcon(sharePetText(p), "pet="+p.id, "Share this pet post")}
      </div>`;
...
function buildSuccessTab(s, uid) {
  ...
  return `<div data-sptab="story">
    ...
    ${homeNote}
    ${shareIcon(shareSuccessText(s), '', 'Share this success story')}
  </div>`;
}
...
function openShareTray(text, deepLink, title) {
  ...
  if (navigator.share) {
    html += `<button class="share-btn" onclick="navigator.share({title:'ImStranded.org',text:'${esc(text).replace(/'/g,"\\'")}',url:'${url}'}).catch(()=>{});closeShareTray()">
      <div class="share-btn-icon" style="background:var(--accent)"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg></div>
  ...
  for (const p of SHARE_PLATFORMS) {
    ...
    if (p.id === 'copy') {
      html += `<button class="share-btn" onclick="navigator.clipboard.writeText('${url}');this.querySelector('.share-btn-label').textContent='Copied!';setTimeout(()=>closeShareTray(),800)">
        <div class="share-btn-icon" style="background:${p.bg};${borderStyle}">${p.icon}</div>
        <span class="share-btn-label">${p.label}</span>
      </button>`;
    } else {
      html += `<button class="share-btn" onclick="window.open('${p.url(text, url)}','_blank','width=600,height=500');closeShareTray()">
        <div class="share-btn-icon" style="background:${p.bg};${borderStyle}">${p.icon}</div>
        <span class="share-btn-label">${p.label}</span>
      </button>`;
    }
...
// Inline share icon HTML — call from any render function
function shareIcon(text, deepLink, title) {
  const safeText = esc(text).replace(/'/g,"\\'").replace(/\n/g,' ');
  const safeTitle = title ? esc(title).replace(/'/g,"\\'") : '';
  const safeDL = deepLink ? esc(deepLink).replace(/'/g,"\\'") : '';
  return `<button class="share-inline" onclick="event.stopPropagation();openShareTray('${safeText}','${safeDL}','${safeTitle}')" title="Share">
...
function sharePetText(p) {
  ...
  const name = p.pet_name ? p.pet_name + ' the ' + type : 'A ' + type;
  return `🐾 ${name} needs a home in ${p.location||'the Gulf'}. Can you help?`;
}
function shareRoomText(p) { return `🏠 Someone in ${p.location||'the Gulf'} is offering a spare room to stranded travelers. Know someone who needs help?`; }
function shareStrandedText(p) { return `🆘 ${p.name||'Someone'} ${p.group_size>1?'('+p.group_size+' people) ':'' }is stranded in ${p.current_location||'the Gulf'} and needs help getting to ${p.destination||'home'}.`; }
function shareAirportText(city, est) { return `✈️ An estimated ${est?.toLocaleString()||'thousands'} people are stranded in ${city}. See the live crisis map.`; }
